...
def plot_figure(df):
    ...
    cp1 = sns.catplot(x="Outcome", y="NIHSS", kind="swarm", data=df, ax=axes[0], order=OUTCOMES)
    plt.sca(axes[0])
    ...
    sns.catplot(x="Outcome", y="Age", kind="swarm", data=df, ax=axes[1], order=OUTCOMES)
    plt.sca(axes[1])
    plt.xlabel("Outcome", fontsize=_xlabel_size)
    ...
    sns.catplot(x="Outcome", y="MRS", kind="box", data=df, ax=axes[2], order=OUTCOMES)
    plt.sca(axes[2])
    plt.xlabel("Outcome", fontsize=_xlabel_size)
