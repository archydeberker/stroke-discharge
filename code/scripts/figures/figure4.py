...
def plot_explanation_for_prediction(model, x, feature_names=feature_names, ax=None):
    ...
    expl.rename(
        {"feature": "Feature", "weight": "Weight", "outcome": "Outcome"},
        inplace=True,
        axis=1,
    )
    ...
    expl["include"][
        expl["Feature"] == "Gender"
    ] = True  # Include Gender as explanation even if value = 0
    g = sns.catplot(
        x="Feature",
        y="Weight",
        hue="Outcome",
...
def plot_prediction(model, x_input, ax=None):
    ...
    df_prediction = pd.DataFrame(
        _pred.T * 100, index=pd.Index(OUTCOME_DICT.values()), columns=["Probability"]
    )
