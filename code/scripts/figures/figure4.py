...
from constants import full_figure_width, figure_height, _xlabel_size, _ylabel_size, OUTCOMES, OUTCOME_DICT
from modelling import fit_and_test_best_model
import pandas as pd

import eli5

feature_names = ['Age', 'MRS0', 'MRS1', 'MRS2', 'MRS3', 'MRS4', 'MRS5', 'NIHSS', 'Gender']
...
def plot_explanation_for_prediction(model, x, feature_names=feature_names, ax=None):
    ...
    g = sns.catplot(x="Feature", y="Weight", hue="Outcome",
                    ...
                    height=6, kind="bar", ax=ax, hue_order=OUTCOMES)
...
def plot_prediction(model, x_input, ax=None):
    ...
    sns.catplot(data=df_prediction, x='Outcome', y='Probability', order=OUTCOMES, kind='bar', ax=ax)
...
if __name__ == '__main__':
    _, best_model, X_test, _ = fit_and_test_best_model()
