...
def run_univariate_stats(df):
    kruskal_results = list(
        map(
            lambda feature: {feature: statistics.compute_kruskal(df, feature)},
            constants.features,
        )
    )
...
def run_correlations_by_outcome(df):
    ...
    for combo in itertools.combinations(constants.features, 2):
        results[combo] = list(
            map(
                lambda outcome: {
                    outcome: statistics.covariate_correlations_segregated_by_outcome(
                        df, combo[0], combo[1], outcome
                    )
                },
                constants.OUTCOMES,
            )
...
def stats_model_comparisons(model_path):
    ...
    results = {
        feature: stats.ttest_ind(collated_scores[(feature,)], dummy)
...
if __name__ == "__main__":
    ...
    model_comparison_results = stats_model_comparisons(
        model_path="../results/all_models_random.p"
    )
